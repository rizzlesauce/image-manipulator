...
GnuplotException::GnuplotException(const std::string &msg) : std::runtime_error(msg){}

inline Gnuplot& Gnuplot::operator<<(const std::string &cmdstr){
   cmd(cmdstr);
   return(*this);
}
    // ----------------------------------------------------------------------
    ...
    /// \return <-- reference to the gnuplot object
    // --------------------------------------------------------------------------
    Gnuplot& Gnuplot::set_hidden3d(){cmd("set hidden3d");return *this;};
...
/*
    ...
    ///  or when generating the same plot for several devices (showonscreen, save)
    ///
    /// \param ---
    ///
    /// \return ---
    //----------------------------------------------------------------------------------
    inline Gnuplot& Gnuplot::replot(){if (nplots > 0) cmd("replot");return *this;};
    */
    // -------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------
    inline bool Gnuplot::is_valid(){return(valid);};
...
//
Gnuplot& Gnuplot::save(const std::string &filename)
{
